...
void ExtractObjects()
{
    ...
    if (linesPnts.size() >= 2)
    {
        ...
        Point intersec;
        size_t j =0;
        sort(cnts_sort.begin(), cnts_sort.end(), sortbysec);
        for (size_t n = 0; n<linesPnts0.size()-1; n++)
        {

                intersection_ = intersection(linesPnts[2*cnts_sort[n+1].x], linesPnts[2*cnts_sort[n+1].x+1], linesPnts[2*cnts_sort[n].x], linesPnts[2*cnts_sort[n].x+1], intersec);
